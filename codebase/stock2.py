...
from codebase.av_utils import getNewsForDates
...
class STOCK(Dataset):
    ...
    def loadData(self, ticker, date, param):
        ...
        for i in range(len(df_norm) - 6):
            df_stock = df_norm.iloc[i:i+6,:]
            df_news = getNewsForDates(ticker, df_stock.index)
            df_all = pd.concat([df_stock.reset_index(drop=True), df_news.reset_index(drop=True)], axis=1)
            df_all.index = df_stock.index
            dataset.append(df_all)
