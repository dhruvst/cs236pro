...
def remove_ah(df, dates):
    dates = [x for x in dates if x.time()>=datetime(2020,1,1,9,30).time() and x.time()<=datetime(2020,1,1,16,0,0).time()]
    df = df[df.date.isin(dates)]
    return df

def remove_short_days(df, dates):
    dates = [x for x in dates if x.time()>=datetime(2020,1,1,9,30).time() and x.time()<=datetime(2020,1,1,16,0,0).time()]
    days = set([x.date() for x in dates])
    for day in days:
        times = [x for x in dates if x.date()==day]
        df1 = df[df.date.isin(times)]
        if len(df1) < 27:
            df = df[~df.date.isin(times)]
    return df

def reformat(df, col):
        '''Input: df: pandas dataframe with stock prices
                col: column to be reformatted (high or low)
           Output: Numpy array of size [batch,27]'''
    df = df[[col]]
    df = df.reset_index().rename(columns={'index':'date'})
    dates = [datetime.strptime(x,'%Y-%m-%d %H:%M:%S') for x in df.date]
    df['date'] = df['date'].apply(lambda x:datetime.strptime(x,'%Y-%m-%d %H:%M:%S'))
    df = remove_ah(df, dates)
    df = remove_short_days(df, dates)
    X = np.array(df[col].astype(float)).reshape(-1,27)
    return X.shape
