...
parser.add_argument('--target_dim', type=int, default=3, help='dimension of target')
parser.add_argument('--input_dim', type=int, default=3, help='dimension of input')
...
d_dfs = dill.load(open('d_dfs_sent_nonan.pkd', 'rb'))
...
for ticker, dates in d_dfs.items():
    for date, df in dates.items():
        ...
        d_temp_predict = {}
        # dates = d_dfs['DG']
        # date, df = list(dates.items())[-1]
        print(ticker,date)
        prices = pd.DataFrame(df.iloc[-2:-1].copy())                #copy for reconstructive from movements

        df['high'] = df['high'].div(df['close'].shift(1))-1         #transform to movements
        df['low'] = df['low'].div(df['close'].shift(1))-1
        df['close'] = df['close'].div(df['close'].shift(1))-1   

        scaler = StandardScaler()                                   #scale to 0 mean 1 var
        scaler.fit(df[['high','low','close']].values)
        ...
        setting = ticker + '_' + train_setting
        exp = Exp(args)  # single experiment
        print('>>>>>>>start training : {}>>>>>>>>>>>>>>>>>>>>>>>>>>'.format(setting))
        exp.train(setting, df.iloc[1:-2], scaler)
        print('>>>>>>>start testing : {}>>>>>>>>>>>>>>>>>>>>>>>>>>'.format(setting))
        preds = exp.test(setting, df.iloc[-args.sequence_length-2:-2], scaler)
        
        preds = scaler.inverse_transform(preds)
        preds = pd.DataFrame(preds + np.ones_like(preds))           #add 1
        preds.columns = ['close', 'high', 'low']

        prices = pd.concat([prices[['close', 'high', 'low']], preds], ignore_index=True) 
        prices['close'] = prices['close'].cumprod()
        prices.loc[1:,'high'] = prices.loc[1:,'high'].mul(prices['close'].shift(1))
        prices.loc[1:,'low'] = prices.loc[1:,'low'].mul(prices['close'].shift(1))
        print(prices.iloc[1:3])
        # all_mse.append(mse)
        torch.cuda.empty_cache()

        d_temp_predict['low'] = prices.loc[1,'low'] #c h l 
        d_temp_predict['high'] = prices.loc[3,'high'] 

        d_predictions[ticker][date] = d_temp_predict
        
        print(d_temp_predict)
        dill.dump(d_predictions,open('dvae_predictions.pkd','wb'))
