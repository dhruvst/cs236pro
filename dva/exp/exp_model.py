# -*-Encoding: utf-8 -*-
from data_load.data_loader import Dataset_Custom, Dataset_df
...
class Exp_Model(object):
    ...
    def _get_data(self, flag, df, scaler):
        args = self.args
        Data = Dataset_df
    ...
    def train(self, setting, df, scaler):
        train_data, train_loader = self._get_data(flag = 'train', df=df, scaler=scaler)
        vali_data, vali_loader = self._get_data(flag = 'val', df=df, scaler=scaler)
        # test_data, test_loader = self._get_data(flag = 'test', df=df)
    ...
    def test(self, setting, df, scaler):
        copy_parameters(self.denoise_net, self.pred_net)
        # test_data, test_loader = self._get_data(flag='test', df=df)
        # preds = []
        # trues = []
        ...
        cols_data = df.columns[:3]
        test_data = df[cols_data]

        seq_x = torch.Tensor(scaler.transform(test_data.values))

        df_stamp = pd.DatetimeIndex(df.index)
        seq_x_mark = torch.Tensor(time_features(df_stamp))

        # for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
        batch_x = seq_x.unsqueeze(0).float().to(self.device)
        # batch_y = batch_y[...,-self.args.target_dim:].float().to(self.device)
        batch_x_mark = seq_x_mark.unsqueeze(0).float().to(self.device)
        noisy_out, out = self.pred_net(batch_x, batch_x_mark)
        # print(out.shape)
        # print(out.shape, batch_y.shape)
        # noisy.append(noisy_out.squeeze(1).detach().cpu().numpy())
        return (out.squeeze((0,1)).detach().cpu().numpy())
